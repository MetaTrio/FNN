...
def main(input_fasta_fastq, type_, input_kmers, model, output):
    ...
    logger.info("Load and prepare input data")
    ...
    # input_data = [np.reshape(row.astype(np.float32), (2080,)) for row in k_mer_arr]  # 6mer
    # input_data = [np.reshape(row.astype(np.float32), (512,)) for row in k_mer_arr]  # 5mer

    # logger.info("accession_numbers")
